...
def main():
    ...
    traceF = np.trace(F)
    print(f"\ndet(A) = {detA:.3f}, сумма диагонали F = {traceF:.3f}")
    if detA > traceF:
        print("det(A) > trace(F) → вычисляем A⁻¹·A − K·F")
        R = np.linalg.inv(A) @ A - K * F
    else:
    ...
    plt.subplot(2, 2, 2)
    diagF = np.diag(F)
    plt.title("Диагональные элементы F")
    plt.bar(range(n), diagF)
    plt.xlabel("Индекс"); plt.ylabel("F[i,i]")
    plt.subplot(2, 2, 3)
    plt.title("Теплокарта матрицы R")
    plt.imshow(R, cmap="plasma")
    plt.colorbar(label="R")
    plt.subplot(2, 2, 4)
    plt.title("Суммы строк матрицы R")
    plt.plot(range(n), R.sum(axis=1), marker="o")
    plt.xlabel("Номер строки"); plt.ylabel("Сумма")
    plt.tight_layout()
    plt.show()
